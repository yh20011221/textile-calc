...
def generate_problem():
    ...
    steps.append(f"⚙️ **編賃の算出**: {total_raw_weight}kg × {knitting_fee}円 = {total_knitting_cost:,.0f}円")
...
if st.session_state.problem:
    ...
    with st.expander("電卓を開く・閉じる"):
        st.caption("Excelのように数式を入力して計算できます")
        calc_input = st.text_input("数式を入力:", placeholder="例: (200 * 0.35 * 1.02) / 22.68")
        if calc_input:
            try:
